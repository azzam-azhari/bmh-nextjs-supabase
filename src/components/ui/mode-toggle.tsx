  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="outline" size="icon" className="w-9 cursor-pointer h-9 bg-background/50 backdrop-blur-md border-border/50 hover:bg-accent/50 transition-all duration-300">
      ...
      <DropdownMenuContent align="end" className="w-40 cursor-pointer backdrop-blur-md bg-background/80">
        <DropdownMenuItem onClick={() => setTheme('light')} className={theme === 'light' ? 'bg-accent/50 font-medium cursor-pointer' : 'cursor-pointer'}>
          <HugeiconsIcon icon={Sun01Icon} className="mr-2 h-4 w-4 text-foreground" />
          <span>Light</span>
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => setTheme('dark')} className={theme === 'dark' ? 'bg-accent/50 font-medium cursor-pointer' : 'cursor-pointer'}>
          <HugeiconsIcon icon={Moon01Icon} className="mr-2 h-4 w-4 text-foreground" />
          <span>Dark</span>
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => setTheme('system')} className={theme === 'system' ? 'bg-accent/50 font-medium cursor-pointer' : 'cursor-pointer'}>
