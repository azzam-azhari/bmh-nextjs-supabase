...
export async function updateBerita(
  ...
  try {
    const hasChanges = formData.get('has_changes') === 'true';
    ...
    if (image instanceof File) {
      ...
      const { error: uploadError } = await supabase.storage
        .from(IMAGE_BUCKET)
        .upload(uploadedImagePath, buffer, {
    ...
    // Hanya perbarui created_at jika ada perubahan nyata pada data berita
    if (hasChanges) {
    ...
    console.error('Error in updateBerita server action:', error);
    return formError(
      previousState,
      error instanceof Error ? error.message : 'Terjadi kesalahan pada server saat memperbarui berita.',
    );
  }
}
